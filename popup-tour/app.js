...
models=await Promise.all(items.map(makeModel));models.forEach((m,i)=>{const [x,,z,h]=plinths[i];m.position.set(x,h+items[i].height/2+.13,z);m.rotation.y=.12;m.userData.item=i;m.traverse(o=>o.userData.item=i);scene.add(m)});
...
r.setAnimationLoop(()=>{
 ...
 r.render(scene,camera);if(dialog.open){vcontrols.update();vr.render(vs,vc)}
})}
