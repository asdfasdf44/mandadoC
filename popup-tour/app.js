...
viewer.addEventListener('pointermove',e=>{if(!dialog.open||!activeModel)return;const rect=viewer.getBoundingClientRect();activeModel.userData.detailTiltX=-((e.clientY-rect.top)/rect.height-.5)*.3;activeModel.userData.detailTiltY=((e.clientX-rect.left)/rect.width-.5)*.34});
viewer.addEventListener('pointerleave',()=>{if(activeModel){activeModel.userData.detailTiltX=0;activeModel.userData.detailTiltY=0}});
