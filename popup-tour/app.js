...
function openItem(i){if(!models.length)return;selected=(i+3)%3;const item=items[selected];if(!dialog.open){lastFocus=document.activeElement;dialog.showModal()}$('#number').textContent=String(selected+1).padStart(2,'0');$('#counter').textContent=`${selected+1} / 3`;$('#product-title').textContent=item.name;$('#category').textContent=item.en;$('#description').textContent=item.desc;if(activeModel)vs.remove(activeModel);activeModel=models[selected].clone();activeModel.position.set(0,0,0);activeModel.rotation.set(0,0,0);activeModel.userData.detailTiltX=0;activeModel.userData.detailTiltY=0;activeModel.scale.setScalar(2/item.height);vs.add(activeModel);vcontrols.autoRotate=false;$('#auto').setAttribute('aria-pressed','false');vc.position.set(0,.08,3.8);vcontrols.target.set(0,0,0);vcontrols.update();resize();$('#close').focus()}
...
vr.domElement.addEventListener('pointermove',e=>{if(!dialog.open||!activeModel)return;const rect=vr.domElement.getBoundingClientRect();activeModel.userData.detailTiltX=-((e.clientY-rect.top)/rect.height-.5)*.52;activeModel.userData.detailTiltY=((e.clientX-rect.left)/rect.width-.5)*.62});
vr.domElement.addEventListener('pointerleave',()=>{if(activeModel){activeModel.userData.detailTiltX=0;activeModel.userData.detailTiltY=0}});
...
r.setAnimationLoop(()=>{
 ...
 const time=performance.now()*.001;models.forEach(m=>{m.position.y=m.userData.floatBaseY+Math.sin(time*.9+m.userData.floatPhase)*.055;m.rotation.x+=((m.userData.tiltX??0)-m.rotation.x)*.12;m.rotation.y+=((m.userData.tiltY??.12)-m.rotation.y)*.12}); r.render(scene,camera);if(dialog.open){if(activeModel){activeModel.position.y=Math.sin(time*1.65)*.15;activeModel.rotation.x+=((activeModel.userData.detailTiltX??0)+Math.sin(time*1.25)*.07-activeModel.rotation.x)*.12;activeModel.rotation.y+=((activeModel.userData.detailTiltY??0)+Math.sin(time*.9)*.18-activeModel.rotation.y)*.12;activeModel.rotation.z=Math.sin(time*1.4)*.08}vcontrols.update();vr.render(vs,vc)}
})}
